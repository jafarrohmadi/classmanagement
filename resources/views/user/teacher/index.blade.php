...
<div class="panel panel-default filter">
    <div class="panel-heading panel-heading-divider">Filter<span class="pull-right"><a href="{{ url('/user/teacher/create') }}" class="btn btn-primary" style="display: unset;"><i class="fa fa-plus"></i> Create</a></span></div>
    <div class="panel-body">
        <form method="POST" id="search-form" class="form-vertical" role="form">
        {{ csrf_field() }}
            <div class="form-group">
                <label for="teacher_nip">Teacher Nip</label>
                <input type="text" class="form-control input-xs" name="teacher_nip" id="teacher_nip" placeholder="search teacher nip">
            </div>
            <div class="form-group">
...
<script type="text/javascript">
    var oTable = $('#dt-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{!! route("user.teacher.data") !!}',
            data: function (d) {
                d.teacher_nip = $('input[name=teacher_nip]').val();
                d.name = $('input[name=name]').val();
            }
        },
        columns: [
            {data: 'teacher_nip', name: 'teacher_nip', orderable: false, searchable: false},
            {data: 'name', name: 'name'},
